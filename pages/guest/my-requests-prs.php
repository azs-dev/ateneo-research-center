...
<div class="container">
  <h2 class="title">My Presentation Requests</h2>         
  <table class="table table-light table-striped">
    ...
    <tbody>
      <tr>
        ...
        $sql = "SELECT * FROM presentation WHERE prs_link= '$id_link' ";
        $result = $conn->query($sql);

        if($result-> num_rows > 0){
          while ($row = $result-> fetch_assoc()) {
            echo "<tr><td>". $row['prs_applicant']."</td><td><a href='my-presentation-view.php?id=".$row['prs_id']."'>". $row['prs_papertitle']."</td><td>". $row['prs_date_sub']."</td><td>". $row['prs_status']."</td></tr>";
          }
        }
        else{
