...
<div class="w3-container w3-animate-top">
  <h2 class="title w3-center">Publication Requests</h2>         
  <table class="table table-light">
    ...
    <tbody>
      <tr>
        <?php
        $sql = "SELECT * FROM publication WHERE pub_visibility = 1";
        $result = $conn->query($sql);

        if($result-> num_rows > 0){
          while ($row = $result-> fetch_assoc()) {
            echo "<tr><td><a href='d-publication-view.php?id=".$row['pub_id']."'>".
