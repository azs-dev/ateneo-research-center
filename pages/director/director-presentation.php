...
<div class="container">
  <h2 class="title w3-center">Presentation Requests</h2>         
  <table class="table table-light">
    ...
    <tbody>
      <tr>
        <?php
        $sql = "SELECT * FROM presentation WHERE prs_visibility=1";
        $result = $conn->query($sql);

        if($result-> num_rows > 0){
          while ($row = $result-> fetch_assoc()) {
            echo "<tr><td><a href='d-presentation-view.php?id=".$row['prs_id']."'>".
                  $row['prs_papertitle']."</td><td>". $row['prs_applicant']."</td><td>".
                  $row['prs_date_sub']."</td><td>". $row['prs_status']."</a></td></tr>";
          }
        }
        else{
